...
# Cheap proxy-side safety disclosure appended only to the stock llama.cpp
# browser/OpenAI-compatible SSE chat stream. Non-streaming JSON/text responses are
# pass-through: the proxy may rewrite the /rag request prompt, but it does not
# mutate non-SSE response bodies. This is deliberately not a regex validator and
# does not skip the LLM call.
APPEND_RAG_FOOTER = os.environ.get("RAG_PROXY_APPEND_FOOTER", "1").strip().lower() not in {"0", "false", "no", "off"}
...
def parse_rag_command(text: str):
    ...
    DEFAULT_RAG_K is resolved only here. Downstream code receives top_k as the
    already-resolved user request. The browser footer later copies that resolved
    value into info["requested_count"] so the footer never needs to know the
    ...
    if not m:
        ...
        if not bare:
            return None
        requested_count = int(bare.group(1) or DEFAULT_RAG_K)
        return {
    ...
    requested_count = int(m.group(1) or DEFAULT_RAG_K)
    return {
        "top_k": requested_count,
...
def build_rag_footer(info: dict[str, Any]) -> str:
    """Return a cheap disclosure for the browser SSE /rag answer.

    The footer is appended by the proxy only for OpenAI-compatible event-stream
    chat responses. It is not generated by the model, does not validate the
    answer, and is never appended to non-streaming JSON/text API responses.

    Keep this as a pure renderer. It must not know DEFAULT_RAG_K: the /rag
    parser resolves defaults once and passes the resolved user request count
    through info["requested_count"].
    """
    selected_count = info.get("selected_count")
    requested_count = info.get("requested_count")
    source_groups_count = info.get("source_groups_count")

    if selected_count is not None and requested_count is not None:
        selection_text = f"selected {selected_count} of {requested_count} requested fragments"
    elif selected_count is not None:
        selection_text = f"selected {selected_count} fragments"
    else:
        selection_text = "used retrieved fragments"

    if source_groups_count is not None:
        selection_text += f" from {source_groups_count} document"
        if source_groups_count != 1:
            selection_text += "s"

    return (
        "\n---\n"
        f"RAG: {selection_text}."
    )


def sse_footer_event(footer: str) -> bytes:
    """Build an OpenAI-compatible streaming delta carrying the proxy footer."""
    payload = {
        "choices": [
            {
                "index": 0,
...
def response_is_event_stream(headers) -> bool:
...
async def maybe_patch_json_body(request_id: str, path: str, data: Any):
    ...
    info["requested_count"] = requested_count

    log(
        request_id,
        "RAG_RETRIEVE "
        f"top_k={info['top_k']} "
        f"requested_count={info['requested_count']} "
        f"pre_k={info['pre_k']} "
    ...
    # Footer text is prepared only for the browser-style chat shape. The stream
    # layer below still appends it only when the upstream response is SSE; all
    # non-streaming JSON/text responses remain byte-for-byte upstream output.
    rag_footer = None
    if APPEND_RAG_FOOTER and detection["shape"] == "messages":
        rag_footer = build_rag_footer(info)

    return patched, True, rag_footer
...
async def lifespan(app: FastAPI):
    ...
    print(f"RAG footer:       {APPEND_RAG_FOOTER}")
...
async def proxy_all(path: str, request: Request):
    ...
    rag_detected = False
    rag_footer: str | None = None

    if should_try_json(content_type, body):
        try:
            data = json.loads(body.decode("utf-8", errors="replace"))
            patched_data, rag_detected, rag_footer = await maybe_patch_json_body(request_id, path, data)
        ...
        if parsed:
            ...
            log(
                ...
                f"requested_count={parsed['top_k']} "
    ...
    async def response_stream():
        try:
            should_append_footer = bool(
                rag_footer
                and APPEND_RAG_FOOTER
                and upstream_response.status_code == 200
                and response_is_event_stream(upstream_response.headers)
            )
            ...
            if should_append_footer:
                ...
                async for line in upstream_response.aiter_lines():
                    if line.strip() == "data: [DONE]":
                        yield sse_footer_event(rag_footer or "")
            ...
            # Non-SSE responses are pass-through. In particular, do not append
            # footer text to non-streaming JSON or plain-text API responses.
